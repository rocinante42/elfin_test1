...
<div
	...
	class="colored-bg-locations md:bg-elfin_yellow md:bg-none fixed md:relative lg:relative z-50 h-full w-full overflow-y-auto "
>
	<div class="flex flex-col pt-6">
		<button class="pl-6 md:invisible lg:invisible md:max-h-0" onclick={onback}><X size={26} strokeWidth={1} /></button>
		<div class="flex md:sticky md:top-0 md:bg-elfin_yellow flex-row items-center justify-between px-6 align-middle">
			<div class="pb-4 pt-8 md:pt-2 lg:pt-2 text-[48px] md:text-[36px] lg:text-[36px]">Locations</div>
			{#if addingLocationState == 'idle'}
				<button
					onclick={addLocation}
					class="h-fit translate-y-2 md:translate-y-[-10px] rounded-full border border-black p-3"><Plus /></button
				>
			{:else}
				<button
					onclick={cancelAddLocation}
					class="h-fit translate-y-2 md:translate-y-[-10px] rounded-full border border-black p-3"><X /></button
				>
			{/if}
		</div>
		{#if addingLocationState == 'adding'}
			<div in:fly={{duration: 500, y: -50}} out:fade={{duration: 100}} class="flex w-full flex-row bg-transparent px-4 py-2">
				<AutoComplete
					tabindex="0"
